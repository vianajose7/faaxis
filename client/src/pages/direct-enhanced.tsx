...
/**
 * Direct Enhanced Calculator Entry Page - Special Page for Bypassing Payment Requirements
 * This page automatically loads example data and redirects to the enhanced calculator
 */
export default function DirectEnhancedPage() {
  const [location, navigate] = useLocation();
  const { setAdvisorInfo, setCalculationResults } = useAdvisorInfo();
  
  // Immediately load data and redirect
  useEffect(() => {
    console.log("DirectEnhancedPage: Loading example data and redirecting...");
    
    // Set sample advisor info with enhanced calculator fields
    setAdvisorInfo({
    ...
    // Set pre-calculated sample results with isPaid flag
    setCalculationResults({
      ...
      isPaid: true // This is crucial - marks the user as having paid access
    });
    
    // Delay navigation slightly to ensure data is set
    const redirectTimer = setTimeout(() => {
      console.log("DirectEnhancedPage: Redirecting to detailed calculator...");
      navigate("/detailed-calculator");
    }, 1500);
    
    return () => clearTimeout(redirectTimer);
  }, [navigate, setAdvisorInfo, setCalculationResults]);
  
  return (
    <div className="flex items-center justify-center min-h-screen bg-background">
      <Card className="p-6 shadow-lg w-[500px] max-w-[90vw] text-center">
        <h1 className="text-2xl font-bold mb-4">Loading Enhanced Calculator</h1>
        <p className="text-muted-foreground mb-6">
          Setting up premium example data and preparing your enhanced dashboard...
        </p>
        ...
        <p className="text-sm text-muted-foreground mb-6">
          You will be automatically redirected to the enhanced calculator in a moment.
        </p>
        
        <Button 
          className="w-full" 
          onClick={() => navigate("/detailed-calculator")}
        >
          Go to Enhanced Calculator Now
        </Button>
      </Card>
    </div>
  );
